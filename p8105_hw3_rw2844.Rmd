...
There are  `r nlevels(pull(instacart_df,aisle))` aisles in the store. And the table above shows the top 5 ordered aisles.       
...
# pick the three most popular aisles, and focus on the target items.
  ...
  knitr::kable()
```
The top three popular products in the specific aisle are shown in the table above. 
...
```
The most observed values are `0` , `25` and `13`, that means there are few snowy days in New York.
